...
启用“暂停查词”后，暂停视频即可看到扩展保存的当前原字幕。英文等以空格分词的字幕点击完整词块会直接查单词，拖选多个词块会直接查短语；日语、中文等字幕使用原生划选，选完即查询，单词或短语类型由同一次 AI 响应判断，不再要求手动选类型。跨句或过长选区会提示重新选择；当前版本不提供句子语法解析或句子卡。单词结果展示释义与视频原句，只有通过基本校验时才显示原形和词形说明；短语结果展示整体含义、可用的简短说明及视频原句。英语、中文、日语的词典规则各自独立；日语优先按辞书形、丁寧形、使役、受身等术语处理，普通名词的活用说明会被隐藏。纯汉字日语与中文无法仅靠字形稳定区分，查日语字幕建议在设置中明确选择日语。其他语言暂沿用通用规则，未做同等微调。词形信息仍由本地模型生成，制卡前请审阅。继续播放、按 Esc 或切换字幕会关闭查词面板；播放期间不会发起查词请求，也不会阻挡播放器操作。

AI 释义是学习辅助，不等同于权威辞书。明显不符合目标语言的释义会在本地重试一次，仍不合格则不显示；这不能识别所有语义错误，制卡前请审阅。查词内容仍只发送到设置中的本地 OpenAI 兼容服务。

自动识别遇到纯汉字字幕时，暂停查词会参考最近的原字幕：若有假名线索，按日语处理。若整个片段都没有假名，汉字本身不足以可靠区分日语与中文；此时请在设置中明确选择源语言。

查词结果会先返回释义；若视频原句译文仍在生成，译文完成后会自动补到词典卡片或尚未填写的 Anki 编辑栏。模型偶尔不按 JSON 格式输出时，扩展会尝试接收简短有效的释义，无法提取时才进行一次简化重试。
