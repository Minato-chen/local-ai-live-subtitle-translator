...
启用“暂停查词”后，暂停视频即可看到扩展保存的当前原字幕。英文等以空格分词的字幕可点击完整词块或拖选多个词块；日语、中文等字幕使用原生划选。选中后直接查询，不要求选择单词或短语类型。结果统一显示选中内容的简短释义（必要时附用法说明）、视频原句及译文；不生成新例句，也不显示词性、原形或语法分析。跨句或过长选区会提示重新选择。继续播放、按 Esc 或切换字幕会关闭查词面板；播放期间不会发起查词请求，也不会阻挡播放器操作。
...
查词只发一次简短释义请求，不要求 JSON。若视频原句译文仍在生成，译文完成后会自动补到查词卡片或尚未填写的 Anki 编辑栏。
