...
- 参考最近一条字幕改善对话语境
- 进入 Netflix 播放页后自动预热当前 Ollama 模型，缩短第一条字幕等待时间
...
进入 `/watch/` 播放页并检测到视频后，扩展会自动发送一个不生成文字的预热请求，
提前把当前 Ollama 模型载入内存。预热期间仍可能有短暂延迟，模型加载完成后字幕会
明显加快。每部影片只预热一次，失败也不会在字幕区域显示错误。
...
2. 进入 `/watch/` 页面并检测到视频后，后台通过空生成请求预热当前 Ollama 模型。
3. 新字幕经过清理、去重和防抖。
4. 扩展保留最近字幕作为可选上下文。
5. `background.js` 调用本机 Ollama 或 LibreTranslate。
6. 日语→中文结果仍含假名或复述原文时，自动严格重试一次。
7. 译文显示在独立覆盖层中，并缓存于当前页面会话；等待期间仅显示 `...`。
