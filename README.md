...
启用“暂停查词”后，暂停视频即可看到扩展保存的当前原字幕。英文等以空格分词的字幕可点击完整词块直接查单词；拖选多个词块会先确认短语。日语、韩语等字幕继续使用原生划选，并在查询前确认选区及“单词／短语”类型。跨句或过长短语会提示重新选择；当前版本不提供句子语法解析或句子卡。词形说明（如过去式）仅在本地模型给出且通过基本校验时显示，不能确定则留空。继续播放、按 Esc 或切换字幕会关闭查词面板；播放期间不会发起查词请求，也不会阻挡播放器操作。
