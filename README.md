...
启用“暂停查词”后，暂停视频即可看到扩展保存的当前原字幕。英文等以空格分词的字幕可点击完整词块或拖选多个词块；日语、中文等字幕使用原生划选。选中后直接查询，不要求选择单词或短语类型。结果采用轻量辞书格式：给出一至两个简短义项，随后显示视频原句及译文。不生成新例句或词形说明。跨句或过长选区会提示重新选择。继续播放、按 Esc 或切换字幕会关闭查词面板；播放期间不会发起查词请求，也不会阻挡播放器操作。

AI 释义是学习辅助，不等同于权威辞书。扩展只检查输出是否为目标语言的简短文字，无法保证语义正确；制卡前请审阅。查词内容仍只发送到设置中的本地 OpenAI 兼容服务。

自动识别遇到纯汉字字幕时，暂停查词会参考最近的原字幕：若有假名线索，按日语处理。若整个片段都没有假名，汉字本身不足以可靠区分日语与中文；此时请在设置中明确选择源语言。

查词先请求简短释义；若模型返回无效释义，最多重试一次。视频原句的译文由当前展示的字幕片段单独生成，不复用播放时参考前文的译文；生成后会补到查词卡片或尚未填写的 Anki 编辑栏。关闭查词面板或继续播放时会取消尚未完成的片段翻译。
...
正面只写选中内容；背面将释义、视频原句及译文合并写入一个字段，不附加出处或时间戳。可按剧集自由选择或手动新建牌组；扩展不会在打开影片时自动创建牌组。卡片正文不会写入浏览器同步存储，只有确认添加时才发送到本机 AnkiConnect。
