...
启用“暂停查词”后，暂停视频即可看到扩展保存的当前原字幕。英文等以空格分词的字幕可点击完整词块查单词，或从一个词拖到另一个词查连续短语；日语、韩语等字幕继续使用原生划选。扩展会使用同一个本地 AI 服务生成简明释义、视频原句语境和一个新的例句。继续播放、按 Esc 或切换字幕会关闭查词面板；播放期间不会发起查词请求，也不会阻挡播放器操作。

AI 释义是学习辅助，不等同于权威辞书。明显不符合目标语言的释义或例句译文会在本地重试一次，仍不合格则不显示；这不能识别所有语义错误，制卡前请审阅。查词内容仍只发送到设置中的本地 OpenAI 兼容服务。
...
正面只写单词；背面将“词性；释义”、视频原句及译文、新例句及译文合并写入一个字段，不附加出处或时间戳。可按剧集自由选择或手动新建牌组；扩展不会在打开影片时自动创建牌组。卡片正文不会写入浏览器同步存储，只有确认添加时才发送到本机 AnkiConnect。
