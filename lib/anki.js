(function (root, factory) {
  ...
  function composeBackHtml(values) {
    ...
    const sections = [];
    if (String(values.lemma || "").trim()) sections.push(`<div>原形：${esc(values.lemma)}</div>`);
    if (String(values.formNote || "").trim()) sections.push(`<div>${esc(values.formNote)}</div>`);
    sections.push(`<div>${esc(meaning)}</div>`);
    if (String(values.usageNote || "").trim()) sections.push(`<div>说明：${esc(values.usageNote)}</div>`);
