(function (root, factory) {
  ...
  function isExamplePlausible(text, code) {
    const profile = get(code);
    return isTextPlausible(text, code) && (!profile?.isExamplePlausible || profile.isExamplePlausible(text));
  }
  return { get, isTextPlausible, isExamplePlausible };
});
