(function (root, factory) {
  ...
  function playbackReference({ title, hostname, seconds, episodeText = "" }) {
    if (!Number.isFinite(seconds) || seconds < 0) return "";
    const name = String(title || "").replace(/\s*[-–—]\s*(Netflix|YouTube)\s*$/i, "").trim().slice(0, 100);
    if (!name) return "";
    const episode = String(episodeText || title || "").match(/\bS(?:eason)?\s*(\d{1,2})\s*E(?:pisode)?\s*(\d{1,3})\b/i);
    const marker = episode && /netflix/.test(hostname || "")
      ? `S${episode[1].padStart(2, "0")}E${episode[2].padStart(2, "0")}` : "";
    const total = Math.floor(seconds);
    const clock = `${Math.floor(total / 3600) > 0 ? `${String(Math.floor(total / 3600)).padStart(2, "0")}:` : ""}${String(Math.floor(total % 3600 / 60)).padStart(2, "0")}:${String(total % 60).padStart(2, "0")}`;
    return [name, marker, `约 ${clock}`].filter(Boolean).join(" · ");
  }
  ...
  return { normalizeSelection, validateLoopbackHttpUrl, escapeHtml, toAnkiHtml, sourceTags, playbackReference, computeFloatingPosition };
});
