(function (root, factory) {
  ...
  const DICTIONARY_SCHEMA = {
    ...
    required: ["term", "normalizedTerm", "pronunciation", "partOfSpeech", "formNote", "definitions", "contextualMeaning", "generatedExample", "generatedExampleTranslation"]
  };
  ...
  function isExamplePlausible(value, language) {
    return profiles.isExamplePlausible(clean(value, 500), language);
  }
  function classifyExamplePair(entry, term, sourceLanguage = "en", targetLanguage = "zh") {
    ...
    if (contains(entry?.generatedExample) && profiles.isExamplePlausible(entry.generatedExample, sourceLanguage)) return { action: "keep" };
    if (contains(entry?.generatedExampleTranslation) && profiles.isExamplePlausible(entry.generatedExampleTranslation, sourceLanguage)) return { action: "swap" };
  ...
  return { DICTIONARY_SCHEMA, DICTIONARY_PROMPT_TEMPLATE, buildDictionaryMessages, buildContextMeaningMessages, preserveSelectedTerm, sanitizeWordForm, dictionaryResponseFormat, parseDictionaryResponse, inferSourceLanguage, isLanguagePlausible, isExamplePlausible, isTargetLanguage, needsDefinitionRepair, isConciseDefinition, isPlausiblePronunciation, containsQueryTerm, sameExample, classifyExamplePair };
});
