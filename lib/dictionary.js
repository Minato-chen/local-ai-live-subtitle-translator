(function (root, factory) {
  ...
  function needsDefinitionRepair(entry, targetLanguage) {
    return !Array.isArray(entry.definitions) || !entry.definitions.some((value) => isTargetLanguage(value, targetLanguage));
  }
  function isPlausiblePronunciation(value) {
    const text = clean(value, 120);
    return !text || (/^[/\[].+[/\]]$/.test(text) && !/[\p{Script=Han}\p{Script=Arabic}]/u.test(text));
  }
  return { DICTIONARY_SCHEMA, DICTIONARY_PROMPT_TEMPLATE, buildDictionaryMessages, dictionaryResponseFormat, parseDictionaryResponse, inferSourceLanguage, isLanguagePlausible, isTargetLanguage, needsDefinitionRepair, isPlausiblePronunciation, classifyExamplePair };
});
