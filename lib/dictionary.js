(function (root, factory) {
  ...
  const DICTIONARY_SCHEMA = {
    ...
    required: ["kind", "term", "normalizedTerm", "pronunciation", "partOfSpeech", "formNote", "definitions", "contextualMeaning"]
  };

  const DICTIONARY_PROMPT_TEMPLATE = [
    "你是双语学习词典。严格按 JSON Schema 返回字段，不增加其他字段。",
    "先判断查询内容是单词还是短语，并在 kind 返回 word 或 phrase。一个完整词条（包括日语活用词形）是 word；固定搭配或多个词组成的表达是 phrase。不能确定时保守处理，不编造词性或原形。",
  ...
  function buildDictionaryMessages({ term, sentence, source, target, kind = "auto" }) {
    ...
    return [
      ...
      { role: "user", content: `查询类型：${kind === "auto" ? "请判断单词或短语" : kind === "phrase" ? "短语" : "单词"}\n查询内容：${clean(term, 80)}\n视频原句：${clean(sentence, 500)}\n释义目标语言：${target || "zh"}\n原句语言：${sourceLanguage}` }
    ];
  }
  ...
  function entryFromMeaning(value, term, targetLanguage = "zh") {
    const meaning = clean(value, 300).replace(/^(?:释义|词义|意思|含义|meaning)\s*[：:]\s*/iu, "").trim();
    if (!clean(term, 80) || !isConciseDefinition(meaning, targetLanguage)) throw new Error("词典返回格式无效");
    return { kind: "", term: clean(term, 80), normalizedTerm: "", pronunciation: "", partOfSpeech: "", formNote: "", definitions: [meaning], contextualMeaning: "" };
  }
  function parseDictionaryResponse(data, targetLanguage = "zh", fallbackTerm = "") {
    ...
    if (start < 0 || end < start) return entryFromMeaning(raw, fallbackTerm, targetLanguage);
    let value;
    try { value = JSON.parse(raw.slice(start, end + 1)); }
    catch {
      try { value = JSON.parse(raw.slice(start, end + 1).replace(/,\s*([}\]])/g, "$1")); }
      catch { return entryFromMeaning(raw, fallbackTerm, targetLanguage); }
    }
    const definitions = (Array.isArray(value.definitions) ? value.definitions : typeof value.definitions === "string" ? [value.definitions] : typeof value.meaning === "string" ? [value.meaning] : [])
      .map((x) => clean(x, 300)).filter(Boolean).slice(0, 5);
    const result = {
      kind: value.kind === "word" || value.kind === "phrase" ? value.kind : "",
  ...
  function resolveLookupKind(requested, reported, term) {
    if (requested === "word" || requested === "phrase") return requested;
    if (/\s/u.test(clean(term, 80))) return "phrase";
    if ([...clean(term, 80)].length > 24) return "phrase";
    return reported === "word" || reported === "phrase" ? reported : "unknown";
  }
  function phraseUsage(entry, targetLanguage) {
    const note = clean(entry?.contextualMeaning, 120);
    const meanings = Array.isArray(entry?.definitions) ? entry.definitions.map((item) => clean(item, 120)).filter(Boolean) : [];
    if (!note || note.length > 80 || !isTargetLanguage(note, targetLanguage) || meanings.some((meaning) => meaning === note || meaning.includes(note) || note.includes(meaning))) return "";
    return note;
  }
  function trustedLemma(entry, term, sourceLanguage, targetLanguage) {
    const lemma = clean(entry?.normalizedTerm, 80);
    if (!lemma || lemma === clean(term, 80) || !entry?.formNote || !isTargetLanguage(entry.formNote, targetLanguage)) return "";
    if (lemma.length > 40 || /[.!?。！？；;]/u.test(lemma) || /\s/u.test(lemma)) return "";
    return isLanguagePlausible(lemma, sourceLanguage) ? lemma : "";
  }
  function inferSourceLanguage(configured, sentence, context = []) {
    ...
    if (/\p{Script=Han}/u.test(text) && !/\p{Script=Latin}/u.test(text)) {
      // Kanji-only lines are ambiguous. A recent kana-bearing subtitle is
      // stronger evidence than the current line's shared Han script.
  ...
  return { DICTIONARY_SCHEMA, DICTIONARY_PROMPT_TEMPLATE, buildDictionaryMessages, buildContextMeaningMessages, preserveSelectedTerm, sanitizeWordForm, dictionaryResponseFormat, parseDictionaryResponse, entryFromMeaning, resolveLookupKind, trustedLemma, phraseUsage, inferSourceLanguage, isLanguagePlausible, isTargetLanguage, needsDefinitionRepair, isConciseDefinition, isPlausiblePronunciation };
});
